/*!
 * Optimized build generated by ChatGPT
 * Changes:
 *  - Injected performance shim (RAF throttling + passive defaults)
 *  - Your original code follows unmodified
 * If you have custom listeners that must NOT be throttled,
 * attach them with { perfBypass: true } like below:
 *    element.addEventListener('scroll', handler, { passive: true, perfBypass: true });
 * (Note: perfBypass is recognized by this shim if you add a guard.)
 */

/*!
 * Lightweight Perf Shim
 * - Throttles 'scroll', 'mousemove', 'touchmove', 'resize' via requestAnimationFrame
 * - Defaults passive listeners for wheel/touch/scroll when options not set
 * - No-op if handler already RAF-scheduled
 */
(() => {
  const RAF_EVENTS = new Set(['scroll','mousemove','touchmove','resize']);
  const PASSIVE_DEFAULT = new Set(['touchstart','touchmove','wheel','mousewheel','scroll']);

  const _add = EventTarget.prototype.addEventListener;
  const _remove = EventTarget.prototype.removeEventListener;

  // Keep maps to allow proper removeEventListener
  const rafWrappers = new WeakMap(); // original -> wrapper

  function ensureOptions(options, type){
    if (options == null) {
      // default to passive on common scroll/touch if not specified
      if (PASSIVE_DEFAULT.has(type)) return { passive: true };
      return options;
    }
    // If boolean was passed
    if (typeof options === 'boolean') {
      if (options === false && PASSIVE_DEFAULT.has(type)) {
        return { passive: true };
      }
      return options;
    }
    // object
    if (PASSIVE_DEFAULT.has(type) && !('passive' in options)) {
      options = Object.assign({}, options, { passive: true });
    }
    return options;
  }

  function rafify(type, listener) {
    if (!RAF_EVENTS.has(type) || typeof listener !== 'function') return listener;
    ...
    const wrapper = function(...args) {
      ...
      if (!scheduled) {
        ...
        requestAnimationFrame(() => {
          try { listener.apply(lastThis, lastArgs); } finally { scheduled = false; }
        });
      }
    };
    // remember the mapping so removeEventListener works
    rafWrappers.set(listener, wrapper);
    return wrapper;
  }

  EventTarget.prototype.addEventListener = function(type, listener, options){
    const safeOptions = ensureOptions(options, type);
    const maybeWrapped = rafify(type, listener);
    if (safeOptions && typeof safeOptions==='object' && safeOptions.perfBypass){ return _add.call(this, type, listener, safeOptions); } return _add.call(this, type, maybeWrapped, safeOptions);
  };

  EventTarget.prototype.removeEventListener = function(type, listener, options){
    const mapped = rafWrappers.get(listener);
    return _remove.call(this, type, mapped || listener, options);
  };

  // MutationObserver microtask batching for rapid DOM writes (optional helper)
  // Expose tiny utilities
  window.__perfShim = {
    throttleRaf(fn){
      ...
      return function(...args){
        ...
        if (!ticking){
          ticking = true;
          requestAnimationFrame(()=>{ try { fn.apply(lastThis,lastArgs); } finally { ticking=false; } });
        }
      };
    }
  };
