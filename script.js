/*! Optimized script generated by ChatGPT - performance enhancements
   - RAF throttling shim applied globally (non-invasive)
   - Cached DOM rects for interactive cards
   - Reduced aurora spots and RAF-batched parallax
   - Passive scroll listeners
*/

(function(){
  // lightweight RAF-throttle helper available at window.__rafThrottle
  window.__rafThrottle = function(fn){
    let scheduled = false, lastArgs, lastThis;
    return function(...args){ lastArgs=args; lastThis=this; if(!scheduled){ scheduled=true; requestAnimationFrame(()=>{ scheduled=false; fn.apply(lastThis,lastArgs); }); } };
  };
})();
...
const useInteractiveCard = () => {
    useEffect(() => {
        ...
        const resizeObserver = new ResizeObserver(() => {
            // Recompute cached rects on resize
            cards.forEach(card => {
        ...
        const updateCardTransform = (card, clientX, clientY) => {
        ...
        const handleMouseMove = (e) => {
            ...
            // Use a per-card RAF to avoid flooding
        ...
        // observe resize to update cached rects
...
const AuroraBackground = () => {
    // Reduce number of spots for performance
    const [spots] = useState(() =>
        Array.from({ length: 8 }).map(() => ({
            top: `${Math.random() * 100}%`,
            left: `${Math.random() * 100}%`,
            size: `${Math.floor(Math.random() * 220 + 180)}px`,
            parallaxFactor: Math.random() * 0.45 + 0.15,
        }))
    );
        ...
        const handleScroll = () => {
            ...
            requestAnimationFrame(() => {
                ...
                spotRefs.current.forEach((ref, i) => {
                    if (ref && ref.current) {
                        ref.current.style.transform = `translateY(${Math.round(scrollY * spots[i].parallaxFactor)}px)`;
                    }
                });
                ticking = false;
            });
        ...
        <div className="aurora-background">
            {spots.map((spot, i) => (
                <div
                    ...
                    className="aurora-spot will-change-transform"
